...
_data = {};
// make sure that _data is not from other databases (collaboration or reports)
...
// This function is one of two major sources of lag in the table. 
// TODO: Look at ways to optimize this behavior.
                ...
                if (!and_functionality) {
                    ...
                    if (filters[i] && filters[i].length > 0) {
                        ...
                        for (var j in filters[i]) {
                            ...
                            } else if (row[tableCols[i]].includes(filters[i][j])) {
                                // console.log((row[tableCols[i]], filters[i][j]))
                                result = true;
                                break
                            }
                        ...
                        // account for NGOs; check if the result matches the 'nonprofit_status' rather than 'organization_category'
                        if (tableCols[i] == 'entity_category' && row[tableCols[i]].toLowerCase().includes('ngo')) {
                            var result = (filters[i].includes(row['entity_category_info']));
                        }
                ...
                // and functionality
                    ...
                    if (filters[i] && filters[i].length > 0) {
                        ...
                        for (var j in filters[i]) {
                            ...
                            } else if (!tableCols[i] == 'entity_category') {
                        ...
                        // account for NGOs being classified by the 'nonprofit_status' field
                        if (tableCols[i] == 'entity_category' && row[tableCols[i]].toLowerCase().includes('ngo')) {
                            var result = (filters[i].includes(row['entity_category_info']));
                        }
...
function sortRecords(records){
    // Change records to an array, since javascript objects DO NOT preserve order.
    // console.log("sortRecords");
    var recordArray = [];
    for (k in records){
        recordArray.push({key: k,
                          record: records[k]});
    }

    // Returns -1, 0, or 1.
    function sortingHelper(r1, r2){
        // Sort alphabetically
        if (_sortMode != "collaboration_links"){
            str1 = r1.record[_sortMode];
            str2 = r2.record[_sortMode];
            return asc(str1, str2);
        }
        // Sort by least number of member collaborations. 
        else{
            str1 = r1.record[_sortMode];
            str2 = r2.record[_sortMode];
            len1 = str1.split("||").length;
            len2 = str2.split("||").length;
            return len1 > len2 ? 1 : (len1 < len2 ? -1 : 0);
        }
    }
    return recordArray.sort(sortingHelper);
}
...
function refreshTableData() {
    ...
    var sortedRecords = sortRecords(recordsToDisplay);

    createTableBody(sortedRecords);
...
// This function is one of two major sources of lag in the table, mostly in the createTableRow function.
// TODO: optimize the createTableRow loop (possibly handle this at some point earlier where we are also looping?)
...
function createTableBody(list) {
    ...
    for (var i = 0; i < list.length; i++){
        stringHelper += createTableRow(list[i].key, list[i].record);
    }
    table.find('tbody:last').append(stringHelper);
...
// Set all header colors to be same as background
function resetHeaderColors(){
    $('th.clickable').each(function(){
        $(this).css('background-color', 'rgba(31, 45, 96, 1)');
    });
}

// Send table refresh only if the mode is actually changed.
...
// Bind behavior to the table headers (static at top).
// On click, highlight the header and change the sorting mode.
$('th.clickable').each(function(){
    $(this).css('cursor', 'pointer');
    $(this).click(function(){
        resetHeaderColors();
        $(this).css('background-color', 'rgba(57, 83, 177, 1)');
        changeSortingMode($(this).attr("data-column"));
    });
});
...
function createFilterFor(key) {
    ...
    // seems to be backup message if the key doesn't exist
    var l = key.indexOf('_') > -1 ? key.indexOf('_') : -1;
    var placeHolder = key.substr(l+1, key.length);
